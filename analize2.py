...
sql1 = "CREATE TABLE vul (Identificador varchar(25), links TEXT, status TEXT, Outros_systemas TEXT, Descrição TEXT, Data_detecao datetime, Avaliação float(5))"
cursor.execute(sql1)

cnxn.commit()
"""
sql2 = "CREATE TABLE vullID_IDS(Identificador varchar(25), links TEXT, status TEXT, Outros_systemas TEXT)"
...
vulFW=vulFW[['Идентификатор','Ссылки на источники','Статус уязвимости', 'Идентификаторы других систем описаний уязвимости']]
vulIPS=vulIPS[['Идентификатор','Ссылки на источники','Статус уязвимости', 'Идентификаторы других систем описаний уязвимости']]
vul=vul[['Идентификатор','Ссылки на источники','Статус уязвимости', 'Идентификаторы других систем описаний уязвимости']]
#print(vul)
vulFW.columns = [c.replace(' ', '_') for c in vul.columns]
vulIPS.columns = [c.replace(' ', '_') for c in vul.columns]
...
def loadDF():
    for index, row in vulFW.iterrows():
        print(row)
        cursor.execute("INSERT INTO vullID_FW (Identificador,links,Status,Outros_systemas) values(%s,%s,%s,%s)", (row.Идентификатор, row.Ссылки_на_источники, row.Статус_уязвимости, row.Идентификаторы_других_систем_описаний_уязвимости))
    cnxn.commit()
    for index, row in vulIPS.iterrows():
        print(row)
        cursor.execute("INSERT INTO vullID_IDS (Identificador,links,Status,Outros_systemas) values(%s,%s,%s,%s)", (row.Идентификатор, row.Ссылки_на_источники, row.Статус_уязвимости, row.Идентификаторы_других_систем_описаний_уязвимости))
    cnxn.commit()
    cursor.close()
